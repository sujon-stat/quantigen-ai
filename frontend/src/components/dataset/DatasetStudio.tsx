            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-300 mt-1">
                      <strong>{totalRows}</strong> observations • <strong>{totalCols}</strong> variables •{' '}
                      <strong>{missingTotal}</strong> missing data points
                    </p>
                  </div>
                </div>
              ...
              <div className="glass-panel p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {cols.map((col: any) => {
                    const role = col.role || (col.data_type === 'continuous' || col.data_type === 'count' ? 'continuous' : col.data_type === 'binary' ? 'binary' : 'categorical');
                          ...
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              ...
                              <div className="text-[11px] text-slate-400">
                                Type: <code className="text-slate-300">{dataType}</code> • Unique: {col.unique_values ?? 0} • Missing: {col.missing_count ?? 0}
                              </div>
                            </div>
                          </div>
                        ...
                        {cols.map((col: any) => {
                          ...
                          return (
                            <th key={col.name} className="p-3 font-semibold">
                              {col.name}{' '}
                              <span className="text-[10px] text-sky-400/80 font-normal">({role})</span>
                            </th>
                          );
                        })}
                      </tr>
                        ...
                        <tr key={idx} className="hover:bg-white/5 transition-colors">
                          {cols.map((col: any) => (
                            <td key={col.name} className="p-3 text-slate-300 font-mono">
                              {row && row[col.name] !== null && row[col.name] !== undefined ? String(row[col.name]) : <span className="text-slate-600 italic">null</span>}
                            </td>
                          ))}
                        </tr>
                      ))}
