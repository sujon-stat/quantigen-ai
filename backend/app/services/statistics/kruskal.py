...
class KruskalWallisMethod(BaseStatisticalMethod):
    ...
    description = "Compares differences across three or more independent groups when the dependent variable is ordinal or continuous but non-normally distributed."
    required_variables = {
    ...
    def run(self, data: pd.DataFrame, variables: Dict[str, Any], options: Optional[Dict[str, Any]] = None) -> MethodResult:
        ...
        if len(dep_vars) > 1 or len(grp_vars) > 1:
                ...
                for g in grp_vars:
                    try:
                        ...
                        multi_table.append({
                            "variable": d,
                            "grouping_column": g,
                            "h_statistic": sub_res.main_results.get("h_statistic", 0.0),
                            "p_value": sub_res.main_results.get("p_value", 1.0),
                            "degrees_of_freedom": sub_res.main_results.get("degrees_of_freedom", 0),
                            "epsilon_squared": sub_res.effect_sizes.get("epsilon_squared", 0.0) if sub_res.effect_sizes else 0.0,
                            "k_groups": sub_res.main_results.get("k_groups", 0)
                        })
                    except Exception as ex:
                        all_warnings.append(f"Could not compute Kruskal-Wallis H for {d} by {g}: {str(ex)}")
            ...
            return MethodResult(
                ...
                main_results={
                    "test_type": "Multi-Variable Kruskal-Wallis H Table",
                ...
                plots=[],
                interpretation=f"Multi-variable Kruskal-Wallis comparison table generated successfully for academic manuscript Table 1 covering {len(dep_vars)} dependent variables across {len(grp_vars)} grouping factors.",
