...
class PortfolioExportRequest(BaseModel):
    title: str = "StatAid Studio Academic Multi-Analysis Portfolio"
...
def export_report(request: ReportExportRequest):
    """Generate a comprehensive, publication-ready StatAid Studio analysis summary report."""
    import re
    ...
    if request.plots_json:
        ...
        for idx, plot_dict in enumerate(request.plots_json):
            ...
            try:
                ...
                """
                doc_mime_parts.append(f"""------=_NextPart_StatAid_MHTML_Boundary
...
{b64_img}""")
    ...
    if request.format == "markdown":
        report_text = f"""# StatAid Studio — Academic Statistical Report: {clean_method_name}
...
---
*Report automatically generated by StatAid Studio — Next-Gen Quantitative Statistical Platform.*
"""
        return Response(
            content=report_text,
            media_type="text/markdown",
            headers={"Content-Disposition": f'attachment; filename="stataid_{safe_method}_report.md"'}
        )
...
<head>
<meta charset="utf-8">
<title>StatAid Studio Manuscript Report: {clean_method_name}</title>
<style>
    @page Section1 {{
        size: 8.5in 11.0in;
...
<body>
<div class="Section1">
<h1>StatAid Studio — Academic Manuscript Report</h1>
...
Content-Type: multipart/related; boundary="----=_NextPart_StatAid_MHTML_Boundary"

------=_NextPart_StatAid_MHTML_Boundary
Content-Type: text/html; charset="utf-8"
Content-Transfer-Encoding: 8bit
Content-Location: file:///C:/stataid_manuscript.htm
...
------=_NextPart_StatAid_MHTML_Boundary--"""
        ...
        return Response(
            ...
            headers={"Content-Disposition": f'attachment; filename="stataid_{safe_method}_manuscript.doc"'}
        )
        ...
        class ManuscriptPDF(FPDF):
            def header(self):
                ...
                self.cell(0, 8, safe_pdf_text("StatAid Studio -- Academic Manuscript Report"), align="R", new_x="LMARGIN", new_y="NEXT")
                self.ln(6)
        ...
        pdf.multi_cell(0, 8, safe_pdf_text("StatAid Studio -- Academic Manuscript Report"), align="C")
        pdf.ln(3)
...
def export_portfolio(request: PortfolioExportRequest):
    ...
    if request.format == "rmarkdown":
        ...
        rmd_lines = [
            ...
            "# Executive Summary",
            "",
            f"This comprehensive statistical portfolio compiles `{len(request.items)}` distinct analytical runs executed via **StatAid Studio**. All models have been automatically validated against underlying distributional assumptions with exact Type I error control ($\alpha = 0.05$).",
            ""
        ]
        ...
        pdf.cell(0, 8, safe_pdf_text("Compiled via StatAid Studio — Hardened Inference & Assumption Shield"), new_x="LMARGIN", new_y="NEXT", align="C")
        pdf.ln(10)
        ...
        body_parts = [f"<h1 style='text-align:center;color:#0f172a;'>{request.title}</h1><p style='text-align:center;color:#64748b;font-style:italic;'>Compiled via StatAid Studio Academic Suite</p><hr>"]
